...
class Solution:
    def numTrees(self, n: int) -> int:
        ...
        for i in range(2, n + 1):  # 遍历从1到n的每个数字
            for j in range(i):  # 对于每个数字i，计算以i为根节点的二叉搜索树的数量
                dp[i] += dp[j] * dp[i-1-j]  # 利用动态规划的思想，累加左子树和右子树的组合数量,比如i=2，总数为dp[0]*dp[1]+dp[1]*dp[0]=2
    ...
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        ...
        for j in range(n+1):    # 遍历背包
            for word in wordDict:    # 遍历物品
                length = len(word)
                if j >= len(word) and s[j-length:j] == word:
                    dp[j] = dp[j] or dp[j-length]
        return dp[n]
```
...
**注意最后的结果可不是dp[nums.size() - 1]！** ，要用result记录最大值。
